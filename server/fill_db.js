...
(async () => {
	try {
		...
		for (const movie of moviesData.movies) {
				try {
			const {rows}= await client.query(`INSERT INTO movies(title, year, runtime, director, actors, plot, "posterUrl")
			VALUES ('${movie.title
			.replace(/'/g)}', 
			${movie.year}, 
			${movie.runtime}, 
			...
			'${movie.actors
			.replace(/'/g)}', 
			'${movie.plot.replace(/"/g).replace(/'/g)}',
			'${movie.posterUrl}'
			) RETURNING id;`)}catch(e) {

			}
			const movieRecord = await client.query(`select * from movies where title='${movie.title
			.replace(/'/g)}'`)
			const movieId = movieRecord.rows[0].id

			for (const genre of movie.genres) {
				try {
					await client.query(`INSERT INTO genres(genre) VALUES('${genre}');`);
				} catch(e) {
				}

				const genreRecord = await client.query(`select * from genres where genre='${genre}'`)

				const genreId = genreRecord.rows[0].id;


				await client.query(`INSERT INTO "moviesGenres"("movieId", "genreId") VALUES(${parseInt(movieId)}, ${parseInt(genreId)});`);
			}
